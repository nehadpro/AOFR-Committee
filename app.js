...
function renderMemberTopics(committee, topics) {
  ...
  root.innerHTML = `
    <div class="main-header">
      <div>
        <h2>لجنة رقم ${esc(committee.number)}</h2>
...
function renderAdminCommittees(main) {
  main.innerHTML = `
    ...
    <div class="card"><div class="table-wrap"><table class="data" id="committeesTable">
      <thead><tr><th>رقم اللجنة</th><th>التاريخ</th><th>الحالة</th><th>عدد الموضوعات</th><th></th></tr></thead>
      <tbody><tr><td colspan="5" class="muted">جارِ التحميل…</td></tr></tbody>
    </table></div></div>`;

  $("#newCommitteeBtn").addEventListener("click", openNewCommitteeModal);
  ...
  const unsub = onSnapshot(qAll, async (snap) => {
    ...
    rows.forEach((c) => {
      ...
      tdActions.append(openBtn, toggleBtn);
...
function openNewCommitteeModal() {
  openModal({
    ...
    bodyHTML: `
      <div class="field"><label>رقم اللجنة</label><input type="text" id="cNumber" placeholder="مثال: ١٤٤٧/١٢" /></div>
      <div class="field"><label>تاريخ الاجتماع</label><input type="date" id="cDate" /></div>`,
    foot: (() => {
      const f = document.createDocumentFragment();
      f.append(
        el("button", { class: "btn", onclick: closeModal }, "إلغاء"),
        (() => {
          const b = el("button", { class: "btn btn-primary" }, "إنشاء");
          b.addEventListener("click", async () => {
            const number = $("#cNumber").value.trim();
            const date = $("#cDate").value;
            if (!number) return toast("الرجاء إدخال رقم اللجنة");
  ...
  main.innerHTML = `
    <div class="main-header">
      <div>
        ...
        <h2 style="margin-top:6px;">لجنة رقم ${esc(committee.number)}</h2>
...
function openMemberModal(member) {
  ...
  openModal({
    ...
    bodyHTML: `
      ...
      ${!member ? '<div class="field"><label>كلمة مرور مبدئية (اختياري)</label><input type="password" id="mPassword" placeholder="يمكن تركها فارغة والدخول بالاسم فقط" /></div>' : ""}
...
function renderAdminReport(main) {
  ...
  getDocs(query(collection(db, "committees"), orderBy("createdAt", "desc"))).then((snap) => {
    ...
    snap.forEach((d) => sel.appendChild(el("option", { value: d.id }, `لجنة رقم ${esc(d.data().number)} — ${fmtDate(d.data().date)}`)));
...
async function buildReport(committeeId) {
  ...
  const html = `
    <div class="report-head">
      ...
      <div class="meta">لجنة رقم ${esc(committee.number)} — ${fmtDate(committee.date)} — الحالة: ${esc(committee.status)}</div>
