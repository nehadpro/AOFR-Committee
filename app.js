...
const TOPIC_CATEGORIES = ["تجديد عقد", "مناقشة قرار سابق", "تعاقد جديد كباحث"];
const QUALIFICATIONS = ["بكالوريوس", "ماجستير", "دكتوراه"];
...
$("#logoutBtn").addEventListener("click", () => {
  ...
  setLoginMode("select");
  memberSelectedCategory = null;
  memberSelectedTopicId = null;
  populateMemberList();
});
...
function renderMemberCommitteePicker(committees) {
  ...
  committees.forEach((c) => {
    const btn = el("button", {
      ...
      onclick: () => { memberCurrentCommitteeId = c.id; memberSelectedCategory = null; memberSelectedTopicId = null; renderMemberCommitteePicker(committees); loadMemberTopics(c); },
    }, esc(c.number));
...
let memberSelectedCategory = null;
function renderMemberTopics(committee, topics) {
  ...
  if (!memberSelectedCategory) {
    const cats = [...TOPIC_CATEGORIES, "غير مصنّف"];
    const cards = cats.map((c) => {
      const count = topics.filter((t) => (t.category || "غير مصنّف") === c).length;
      return { name: c, count };
    }).filter((c) => c.count > 0);
    ...
    root.innerHTML = `
      ...
      <div class="card card-pad">
        <div class="text-sm muted" style="margin-bottom:12px;">اختر فئة الموضوعات لعرض قائمة الباحثين ضمنها</div>
        <div id="categoryCards" style="display:flex; flex-wrap:wrap; gap:12px;"></div>
      </div>`;
    const box = $("#categoryCards");
    cards.forEach((c) => {
      const card = el("button", {
        class: "btn", style: "padding:14px 22px; font-size:14px;",
        onclick: () => { memberSelectedCategory = c.name; renderMemberTopics(committee, topics); },
      }, `${esc(c.name)} (${c.count})`);
  ...
  const filteredTopics = topics.filter((t) => (t.category || "غير مصنّف") === memberSelectedCategory);
  if (!memberSelectedTopicId || !filteredTopics.find((t) => t.id === memberSelectedTopicId)) {
    memberSelectedTopicId = filteredTopics.find((t) => !(t.decisions && t.decisions[CURRENT.id]))?.id || filteredTopics[0]?.id;
  }

  root.innerHTML = `
    <div class="main-header">
      <div>
        <button class="btn btn-ghost btn-sm" id="backToCategories">‹ كل التصنيفات</button>
        <h2 style="margin-top:6px;">${esc(committee.number)} — ${esc(memberSelectedCategory)}</h2>
        <div class="sub">${fmtDate(committee.date)} · <span class="status-chip status-active">مفتوحة للتصويت</span></div>
      </div>
      <div class="main-actions"><div class="text-sm muted">أنجزت ${decided} من ${topics.length} موضوعًا</div></div>
    </div>
  ...
  $("#backToCategories").addEventListener("click", () => { memberSelectedCategory = null; renderMemberTopics(committee, topics); });
  ...
  const topic = filteredTopics.find((t) => t.id === memberSelectedTopicId);
  ...
  box.innerHTML = `
    <div class="dossier-head">
      ...
      <div class="row" style="gap:8px;">
        ${topic.category ? `<span class="badge">${esc(topic.category)}</span>` : ""}
...
function renderAdminMain() {
  ...
  if (adminView === "stats") return renderAdminStats(main);
...
function renderAdminTopicsTable(main, committee, topics, totalMembers) {
  ...
  topics.forEach((t, i) => {
    ...
    tr.innerHTML = `
      <td></td>
      <td>${i + 1}</td>
      <td>${esc(t.researcherName || "—")}${t.category ? `<div class="text-sm muted">${esc(t.category)}</div>` : ""}<div class="text-sm muted">الرقم الوظيفي: ${esc(t.fileNumber || "—")}</div></td>
      <td>${esc(t.entity || "—")}</td>
  ...
  openModal({
    ...
    bodyHTML: `
      <div class="form-grid">
        ...
        <div class="field full"><label>فئة الموضوع</label><select id="fCategory">
          <option value="" ${!t.category ? "selected" : ""}>— اختر الفئة —</option>
          ${TOPIC_CATEGORIES.map((c) => `<option value="${esc(c)}" ${t.category === c ? "selected" : ""}>${esc(c)}</option>`).join("")}
        </select></div>
        ...
        <div class="field"><label>المؤهل</label><select id="fQualification">
          <option value="" ${!t.qualification ? "selected" : ""}>— اختر —</option>
          ${QUALIFICATIONS.map((q) => `<option value="${esc(q)}" ${t.qualification === q ? "selected" : ""}>${esc(q)}</option>`).join("")}
        </select></div>
...
const CHART_PALETTE = ["#2f4b3c", "#a9812f", "#8b3a3a", "#9c6b1f", "#5b7c6c", "#c2a25a", "#6b6f76", "#7a9285"];

function finalDecisionValue(topic) {
  const { best } = tallyDecisions(topic);
  return topic.finalDecisionOverride || best;
}

let statsChartInstances = {};
function renderChart(canvasId, config) {
  if (statsChartInstances[canvasId]) statsChartInstances[canvasId].destroy();
  const canvas = $("#" + canvasId);
  if (!canvas) return;
  statsChartInstances[canvasId] = new Chart(canvas.getContext("2d"), config);
}

async function renderAdminStats(main) {
  main.innerHTML = `
    <div class="main-header"><div><h2>الإحصائيات</h2><div class="sub">نظرة تحليلية على قرارات اللجنة عبر اجتماعاتها المختلفة</div></div></div>
    <div class="card card-pad">
      <div class="field" style="max-width:340px;"><label>النطاق</label>
        <select id="statsScope"><option value="all">كل اللجان (تجميعي)</option></select>
      </div>
      <div id="statsEmptyState" class="empty-state hidden"><div class="display">لا توجد بيانات كافية بعد</div><div class="text-sm">أضف لجانًا وموضوعات أولًا لعرض الإحصائيات</div></div>
    </div>
    <div id="statsCharts">
      <div class="card card-pad">
        <h3 style="font-size:15px; margin-bottom:14px;">توزيع القرارات النهائية</h3>
        <div style="max-width:420px; margin:0 auto;"><canvas id="chartDecisions" height="240"></canvas></div>
      </div>
      <div class="card card-pad">
        <h3 style="font-size:15px; margin-bottom:14px;">عدد المواضيع حسب فئة الموضوع</h3>
        <canvas id="chartCategories" height="180"></canvas>
      </div>
      <div class="card card-pad">
        <h3 style="font-size:15px; margin-bottom:14px;">أكثر الجهات تكرارًا في الموضوعات</h3>
        <canvas id="chartEntities" height="200"></canvas>
      </div>
      <div class="card card-pad" id="trendCardWrap">
        <h3 style="font-size:15px; margin-bottom:14px;">مقارنة القرارات النهائية عبر اجتماعات اللجنة</h3>
        <canvas id="chartTrend" height="220"></canvas>
      </div>
    </div>`;

  const committeesSnap = await getDocs(query(collection(db, "committees"), orderBy("createdAt", "asc")));
  const committees = committeesSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
  const sel = $("#statsScope");
  committees.forEach((c) => sel.appendChild(el("option", { value: c.id }, esc(c.number))));

  const topicsByCommittee = {};
  for (const c of committees) {
    const tSnap = await getDocs(collection(db, "committees", c.id, "topics"));
    topicsByCommittee[c.id] = tSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
  }
  const allTopics = Object.values(topicsByCommittee).flat();

  if (!committees.length || !allTopics.length) {
  ...
  function currentTopics() {
    ...
    return scope === "all" ? allTopics : (topicsByCommittee[scope] || []);
  }

  function drawDecisionsChart(topics) {
    const counts = {}; DECISION_OPTIONS.forEach((o) => (counts[o.value] = 0));
    let undecided = 0;
    topics.forEach((t) => { const v = finalDecisionValue(t); if (v && counts[v] !== undefined) counts[v]++; else undecided++; });
    const labels = DECISION_OPTIONS.map((o) => o.label).concat(undecided ? ["لم يُحسم بعد"] : []);
    const data = DECISION_OPTIONS.map((o) => counts[o.value]).concat(undecided ? [undecided] : []);
    const colors = DECISION_OPTIONS.map((o) => DECISION_COLORS[o.value]).concat(undecided ? ["#d8d3c2"] : []);
    renderChart("chartDecisions", {
      type: "doughnut",
  ...
  function drawCategoriesChart(topics) {
    const cats = [...TOPIC_CATEGORIES, "غير مصنّف"];
    const counts = cats.map((c) => topics.filter((t) => (t.category || "غير مصنّف") === c).length);
    renderChart("chartCategories", {
      type: "bar",
      data: { labels: cats, datasets: [{ label: "عدد المواضيع", data: counts, backgroundColor: CHART_PALETTE[0], borderRadius: 4 }] },
      options: { plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { precision: 0 } } } },
    });
  }

  function drawEntitiesChart(topics) {
    const byEntity = {};
    topics.forEach((t) => { const e = t.entity || "غير محدد"; byEntity[e] = (byEntity[e] || 0) + 1; });
    const sorted = Object.entries(byEntity).sort((a, b) => b[1] - a[1]).slice(0, 8);
    renderChart("chartEntities", {
      type: "bar",
      data: { labels: sorted.map((x) => x[0]), datasets: [{ label: "عدد المواضيع", data: sorted.map((x) => x[1]), backgroundColor: CHART_PALETTE[1], borderRadius: 4 }] },
      options: { indexAxis: "y", plugins: { legend: { display: false } }, scales: { x: { beginAtZero: true, ticks: { precision: 0 } } } },
    });
  }

  function drawTrendChart() {
    const labels = committees.map((c) => c.number);
    const datasets = DECISION_OPTIONS.map((o) => ({
      label: o.label,
      backgroundColor: DECISION_COLORS[o.value],
      data: committees.map((c) => (topicsByCommittee[c.id] || []).filter((t) => finalDecisionValue(t) === o.value).length),
    }));
  ...
  function draw() {
    ...
    drawDecisionsChart(topics);
    drawCategoriesChart(topics);
    drawEntitiesChart(topics);
    const isAll = sel.value === "all";
    $("#trendCardWrap").classList.toggle("hidden", !isAll || committees.length < 2);
    if (isAll && committees.length >= 2) drawTrendChart();
  }
...
function renderAdminReport(main) {
  main.innerHTML = `
    <div class="main-header"><div><h2>التقارير والطباعة</h2><div class="sub">اختر لجنة لعرض قراراتها وطباعتها</div></div></div>
    <div class="card card-pad">
  ...
  getDocs(query(collection(db, "committees"), orderBy("createdAt", "desc"))).then((snap) => {
    ...
    sel.addEventListener("change", () => sel.value && buildReportPreview(sel.value));
  });
}

function buildReportPreview(committeeId) {
  $("#reportPreview").innerHTML = `
    <div class="card card-pad" style="margin-top:18px;">
      <div class="field" style="max-width:320px;">
        <label>نوع التقرير</label>
        <select id="reportTypeSelect">
          <option value="summary">ملخص القرارات</option>
          <option value="detailed">تقرير تفصيلي (يشمل قرار كل عضو)</option>
        </select>
      </div>
      <div id="reportBody"></div>
    </div>`;
  const typeSel = $("#reportTypeSelect");
  typeSel.addEventListener("change", () => buildReport(committeeId, typeSel.value));
  buildReport(committeeId, "summary");
}

async function buildReport(committeeId, reportType) {
  ...
  const html = reportType === "detailed"
    ? buildDetailedReportHTML(committee, topics, await getRosterForReport())
    : buildSummaryReportHTML(committee, topics);

  const label = reportType === "detailed" ? "طباعة التقرير التفصيلي" : "طباعة الملخص";
  const hint = reportType === "detailed"
    ? '<div class="hint" style="margin-top:8px;">نظرًا لكثرة الأعمدة، يُفضّل اختيار الاتجاه "أفقي / Landscape" في نافذة الطباعة للحصول على أفضل نتيجة.</div>'
    : "";
  $("#reportBody").innerHTML = `${html}
    <div style="margin-top:16px;"><button class="btn btn-primary" id="printBtn">${label}</button></div>${hint}`;
  $("#printReport").innerHTML = html;
  $("#printBtn").addEventListener("click", () => window.print());
}

async function getRosterForReport() {
  const snap = await getDocs(collection(db, "members"));
  return sortMembersForDisplay(snap.docs.map((d) => ({ id: d.id, ...d.data() }))).filter((m) => m.active !== false);
}

function buildSummaryReportHTML(committee, topics) {
...
function decisionCellLabel(d) {
  if (!d) return '<span class="text-sm muted">قيد التصويت</span>';
  const opt = optionByValue(d.decision);
  const label = d.decision === "other" && d.otherText ? d.otherText : (opt ? opt.label : d.decision);
  return esc(label);
}

function buildDetailedReportHTML(committee, topics, roster) {
  const memberHeaders = roster.map((m) => `<th>${esc(m.name)}</th>`).join("");
  const rows = topics.map((t, i) => {
    const { best, tie, total } = tallyDecisions(t);
    const finalVal = t.finalDecisionOverride || best;
    const finalOpt = finalVal ? optionByValue(finalVal) : null;
    const decisions = t.decisions || {};
    const memberCells = roster.map((m) => `<td>${decisionCellLabel(decisions[m.id])}</td>`).join("");
    return `<tr>
      <td>${i + 1}</td>
      <td>${esc(t.researcherName || "—")}</td>
      <td>${esc(t.fileNumber || "—")}</td>
  ...
  return `
    <div class="report-head">
      <img src="ksu-logo.png" alt="شعار جامعة الملك سعود" style="height:56px; margin-bottom:10px;" />
      <h1>تقرير تفصيلي بقرارات اللجنة الدائمة لتنفيذ آلية التجديد للمتعاقدين</h1>
      <div class="meta">${esc(committee.number)} — ${fmtDate(committee.date)} — الحالة: ${esc(committee.status)}</div>
      <div class="meta">على برنامج الاستقطاب</div>
    </div>
    <table class="report-table" style="font-size:10.5px;">
      <thead><tr><th>#</th><th>اسم الباحث</th><th>الرقم الوظيفي</th>${memberHeaders}<th>القرار النهائي</th><th>عدد الأصوات</th></tr></thead>
      <tbody>${rows || `<tr><td colspan="${5 + roster.length}">لا توجد موضوعات</td></tr>`}</tbody>
    </table>
    <div class="report-signoff">
      <div>توقيع رئيس اللجنة: ______________________</div>
      <div>التاريخ: ______________________</div>
