...
async function checkPasswordFieldVisibility(member) {
  $("#memberPasswordField").style.display = member && member.passwordHash ? "flex" : "none";
}
...
if (memberUsernameInput) {
  memberUsernameInput.addEventListener("blur", async () => {
    ...
    checkPasswordFieldVisibility(snap.empty ? null : snap.docs[0].data());
  });
  memberUsernameInput.addEventListener("input", () => { if (memberUsernameInput.value.trim()) memberSelect.value = ""; });
  memberSelect.addEventListener("focus", () => { memberUsernameInput.value = ""; });
}
...
memberSelect.addEventListener("change", async () => {
  ...
  checkPasswordFieldVisibility(snap.data());
});

$("#memberLoginForm").addEventListener("submit", async (e) => {
  e.preventDefault();
  hideLoginError();
  const typedUsername = memberUsernameInput ? memberUsernameInput.value.trim() : "";
  let id, m;
  if (typedUsername) {
    const snap = await getDocs(query(collection(db, "members"), where("username", "==", typedUsername)));
    ...
    if (!id) return showLoginError("الرجاء اختيار اسمك أو كتابة بريدك الجامعي");
    const snap = await getDoc(doc(db, "members", id));
    if (!snap.exists()) return showLoginError("تعذر العثور على هذا العضو");
    m = snap.data();
  }
  if (m.active === false) return showLoginError("هذا الحساب معطّل حاليًا");
  if (m.passwordHash) {
...
function renderAdminMembers(main) {
  main.innerHTML = `
    <div class="main-header">
      <div><h2>الأعضاء</h2><div class="sub">رئيس اللجنة والأعضاء المصوّتون — الترتيب هنا هو نفس ترتيب ظهور الأسماء عند تسجيل الدخول</div></div>
  ...
  const unsub = onSnapshot(collection(db, "members"), (snap) => {
    ...
    members.forEach((m, i) => {
      ...
      const upBtn = el("button", { class: "icon-btn", title: "تحريك لأعلى", style: i === 0 ? "opacity:.3; cursor:default;" : "" }, "▲");
      const downBtn = el("button", { class: "icon-btn", title: "تحريك لأسفل", style: i === members.length - 1 ? "opacity:.3; cursor:default;" : "" }, "▼");
